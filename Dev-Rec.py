...
def main():
    ...
    Lower_green = np.array([100, 60, 60])  # np.array([110, 50, 50])
    Upper_green = np.array([140, 255, 255])  # np.array([130, 255, 255])
    pred_class = 0
    pts = deque(maxlen=512)
    #pts = deque(maxlen=812)
        ...
        if len(cnts) >= 1:
            ...
            if cv2.contourArea(contour) > 250:
                ...
                for i in range(1, len(pts)):
                    if pts[i - 1] is None or pts[i] is None:
                        continue
                    cv2.line(blackboard, pts[i - 1],
                             pts[i], (255, 255, 255), 10)
            ...
            if len(pts) != []:
                ...
                thresh1 = cv2.threshold(
                    blur1, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
                blackboard_cnts, _ = cv2.findContours(
